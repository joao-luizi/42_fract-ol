...
/*   Updated: 2024/11/19 14:38:21 by joaomigu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int user_color_a(int x, int y, t_state *s)
{
	...
	if (color_a_area_r(x, y, &r) 
		|| color_a_area_g(x, y, &g)
		|| color_a_area_b(x, y, &b))
	{
		s->f->color_a_edit = r << 16 | g << 8 | b;
		ft_printf("Color_a returns TRUE %d %d\n", x, y);
		return (TRUE);
	}
	ft_printf("Color_a returns FALSe %d %d\n", x , y);
	return (FALSE);
